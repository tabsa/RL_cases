...
class p2p_env:
    def __init__(self, agents, ag_parameter, no_time, variance=False):  # Define parameters of the environment
        self.agents = agents  # Array of p2p_agent_n, each n represent a prosumer j to trade energy
        self.ag_parameter = ag_parameter # Class that defines the Characteristics of each prosumer j
        ...
        self.action_size = len(agents)  # Size of the action space - no of prosumer j
...
class Agents_characteristics:
    def __init__(self, agents, no_time): # Define the paramters of this class
        self.no_agents = len(agents)
        self.energy = np.zeros((self.no_agents, no_time))


#%% Build the prosumer_i
class prosumer_i:
